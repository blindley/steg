...
//
// The two 8 byte groups become the magic chunks, which are used to replace the first usable chunk
// in the first usable bitplane, according to the users specification. Because these chunks contain
// 14 randomly generated bytes, or 112 bits, the probability of these bytes appearing by chance in
// an image is astronomically low. So the presence of these magic chunks does two things. First, it
// identifies the image as almost certainly containing a hidden message which was inserted by this
// program. And second, it allows the extraction algorithm determine which bitplanes were used for
// hiding the message.
        ...
        for (size_t ci = 0; ci < chunks_per_bitplane; ci++) {
            ...
            if (complexity >= threshold) {
                ...
                if (magic_chunk_index < 2) {
                    cover_chunk = magic_chunks[magic_chunk_index++];
                } else {
