...
//
// SIG14 is a const array consisting of 14 random bytes generated from random.org These 14 bytes are
// split into 2 groups of 7, and then an 8th byte is attached to each. These appended bytes actually
// contain 4 numbers, taking 4 bits each, indicating how many bitplanes of each of the channels is
// used for hiding the message.
//
// The two 8 byte groups become the signature chunks, which are used to replace the first usable
// chunk in the first usable bitplane, according to the users specification. Because these chunks
// contain 14 randomly generated bytes, or 112 bits, the probability of these bytes appearing by
// chance in an image is astronomically low. So the presence of these signature chunks does two
// things. First, it identifies the image as almost certainly containing a hidden message which was
// inserted by this program. And second, it allows the extraction algorithm determine which
// bitplanes were used for hiding the message.
...
// signature chunks (see generate_signature_chunks(...))
